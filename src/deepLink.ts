 ...
 * still in flight). Absent / empty params are a silent no-op. A present
 * but unknown or invalid id never throws and never fetches NOAA/NWPS
 * for that id — `onGaugeMissing` opens the not-found panel instead.
 */
export async function handleDeepLink(
  ...
  onGaugeMissing?: (app: GeoLibreAppAPI, lid: string) => void,
): Promise<void> {
  const lid = params.get(DEEP_LINK_PARAM);
  if (!lid) return;

  if (!GAUGE_ID_RE.test(lid)) {
    onGaugeMissing?.(app, lid);
    return;
  }
